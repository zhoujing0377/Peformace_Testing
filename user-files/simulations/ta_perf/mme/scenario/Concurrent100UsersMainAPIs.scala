...
class Concurrent100UsersMainAPIs extends Simulation {
...
val scn = scenario("100 concurrent users comes to customer site")
...
.exec(http("homepage_campaign_GET")
    ...
    .exec(http("E_class_GET")
            .get(mmeHost +"api/ecommerce/customers/vehicles/classes/213")
            .headers(dcpApiHeader)
            )

    .exec(http("E_IPD_GET")
            .get(mmeHost + "api/ecommerce/customers/vehicles/classes/213/demonstration")
            .headers(dcpApiHeader)
            )

    .exec(http("E_PDP_CONTENT_GET")
            .get(mmeHost + "api/ecommerce/customers/contents/pdp?classId=213")
            .headers(dcpApiHeader)
            )
